...
test('Test 3: Todo List', async ({ page }) => {
    ...
    await test.step('b. Xóa các todo có sổ lẻ', async () => {
        ...
        //Kiểm tra todo list chỉ còn hiện các todo số chẵn, todo số lẻ sẽ hidden
        for (let i = 1; i <= 100; i++) {
            if (i % 2 === 0) {
                await expect(page.locator(`//li[span[text()='Todo ${i}']]`)).toBeVisible();
            } else {
                await expect(page.locator(`//li[span[text()='Todo ${i}']]`)).toBeHidden();
            }
        }
    });
});